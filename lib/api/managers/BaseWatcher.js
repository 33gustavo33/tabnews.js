...
function checkWhatHasChanged(data1, data2, observeWhat, defaultObserveWhat) {
  ...
  for (let itemToObserve of observeWhat) {
    ...
    if (checkIfValueExists(value1) && defaultObserveWhat.includes(itemToObserve) && !result[itemToObserve]) {
      //check if the itemToObserve is valid
...
class BaseWatcher extends EventManager {
  #config = {};
  #lastValues = {};
  #observeWhat = [];
