...
export default class {
   ...
   * @param {string} body
   */
  logRequest(url, { method, token, body }) {
    if (!this.#config.debug) return;
    if (!url || !method) return;

    const bodyString = body ? JSON.stringify(body) : "no body";
    this.log(
      `Api: REQUEST ${method ?? "GET"} TO ${url}. With token: ${token ?? "no token"} - With body: ${bodyString}`,
      "green"
    );
  }
}
